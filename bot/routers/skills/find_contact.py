...
async def enter_fio_employee(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.message.answer(f'Выбрано: {emojis.SUCCESS} Найти контакты сотрудника')
...
async def send_data(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    try:
        ...
        order_message = await helper.fill_event_data(message.text, message.from_user.full_name, message.from_user.username, name_order, 0)
