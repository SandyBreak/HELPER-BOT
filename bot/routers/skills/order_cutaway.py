...
from data_storage.states import OrderCutway
...
async def enter_fio_employee(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(OrderCutway.enter_type_office)


@router.message(F.text, StateFilter(OrderCutway.enter_type_office))
