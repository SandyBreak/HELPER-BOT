...
async def send_data(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    try:
        ...
        order_message = await helper.fill_event_data(message.text, message.from_user.full_name, message.from_user.username, name_order, 6)
        ...
        if success_flag:
            await message.answer(f'{emojis.SUCCESS} Запрос на вызов технического специалиста успешно отправлен, при необходимости с вами свяжутся', reply_markup=ReplyKeyboardRemove())
