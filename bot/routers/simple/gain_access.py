...
from models.states import GainAccess
...
async def enter_office(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    await CreateEventService.init_new_event(callback.from_user.id, 'gain_access')
    
    office_keyboard = await UserKeyboards.ultimate_keyboard('room')
    delete_message = await bot.edit_message_text(chat_id=callback.message.chat.id, message_id=callback.message.message_id, text=f"{Emojis.SUCCESS} Получить доступ {Emojis.SUCCESS}\nВыберите офис в котором вы находитесь:", reply_markup=office_keyboard.as_markup(resize_keyboard=True))

    await state.update_data(message_id=delete_message.message_id)
    await state.set_state(GainAccess.get_info)


@router.callback_query(F.data, StateFilter(GainAccess.get_info))
async def enter_type_access(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    if data['key'] == 'choice':
        ...
        delete_message = await callback.message.answer(f'Введите информацию о доступе который вам необходимо предоставить')
...
async def send_data(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    order_message = await MinorOperations.fill_simple_event_data(office, message.from_user.full_name, message.from_user.username, message.text, 'gain_access')
    ...
    try:
        ...
        await message.answer(f'{Emojis.SUCCESS} Запрос на предоставление доступа успешно отправлен, при необходимости с вами свяжутся')
