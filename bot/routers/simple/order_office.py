...
async def enter_office(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    await CreateEventService.init_new_event(callback.from_user.id, 'order_office')
    
    office_keyboard = await UserKeyboards.ultimate_keyboard('room')
    delete_message = await bot.edit_message_text(chat_id=callback.message.chat.id, message_id=callback.message.message_id, text=f"{Emojis.SUCCESS} Заказать канцелярию {Emojis.SUCCESS}\nВыберите офис в котором вы находитесь:", reply_markup=office_keyboard.as_markup(resize_keyboard=True))
...
async def enter_product(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    if data['key'] == 'choice':
        ...
        delete_message = await callback.message.answer(f'Введите товары которые нужно заказать', reply_markup=ReplyKeyboardRemove())
...
async def send_data(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    order_message = await MinorOperations.fill_simple_event_data(office, message.from_user.full_name, message.from_user.username, message.text, 'order_office')
    ...
    try:
        ...
        await message.answer(f'{Emojis.SUCCESS} Запрос на заказ канцелярии успешно отправлен, при необходимости с вами свяжутся')
