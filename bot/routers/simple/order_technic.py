...
async def enter_office(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    await CreateEventService.init_new_event(callback.from_user.id, 'order_technic')
    
    office_keyboard = await UserKeyboards.ultimate_keyboard('room')
    delete_message = await bot.edit_message_text(chat_id=callback.message.chat.id, message_id=callback.message.message_id, text=f"{Emojis.SUCCESS} Заказать/отремонтировать технику {Emojis.SUCCESS}\nВыберите офис в котором вы находитесь:", reply_markup=office_keyboard.as_markup(resize_keyboard=True))
...
async def enter_fio_employee(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    if data['key'] == 'choice':
        ...
        breaks_keyboard = UserKeyboards.breaks_keyboard()
        delete_message = await callback.message.answer(f'Выберите тип поломки при помощи предложенной клавиатуры или введите его вручную', reply_markup=breaks_keyboard.as_markup(resize_keyboard=True))
...
async def send_data(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    order_message = await MinorOperations.fill_simple_event_data(office, message.from_user.full_name, message.from_user.username, message.text, 'order_technic')
    ...
    try:
        ...
        await message.answer(f'{Emojis.SUCCESS} Запрос на вызов технического специалиста успешно отправлен, при необходимости с вами свяжутся')
