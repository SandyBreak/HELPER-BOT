...
from models.states import OrderCutway
...
async def enter_office(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    await state.set_state(OrderCutway.get_info)


@router.callback_query(F.data, StateFilter(OrderCutway.get_info))
async def enter_type_access(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    if data['key'] == 'choice':
        ...
        delete_message = await callback.message.answer(f'Введите ФИО сотрудника для которого вам нужна визитка:')
    
        await state.update_data(message_id=delete_message.message_id)
        await state.set_state(OrderCutway.send_order)

        
        
@router.message(F.text, StateFilter(OrderCutway.send_order))
async def send_data(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    order_message = await MinorOperations.fill_simple_event_data(office, message.from_user.full_name, message.from_user.username, message.text, 'order_cutaway')
    ...
    try:
        ...
        await message.answer(f'{Emojis.SUCCESS} Запрос на создание визитки успешно отправлен, при необходимости с вами свяжутся')
